...
const version=process.env.TEMPLE_RELEASE_VERSION||packageVersion;
const directory=`.release/v${version}`,manifest=JSON.parse(await fs.readFile(`${directory}/manifest.json`,'utf8'));
if(manifest.version!==version)throw new Error('Release version does not match requested source version');
