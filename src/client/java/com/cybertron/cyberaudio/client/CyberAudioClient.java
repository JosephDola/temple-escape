...
public final class CyberAudioClient implements ClientModInitializer {
    ...
    @Override public void onInitializeClient() {
        CONFIG.load(); AUDIO.setVolume(CONFIG.config().volume);
        KeyMapping.Category category = KeyMapping.Category.register(ResourceLocation.fromNamespaceAndPath(CyberAudio.MOD_ID, "controls"));
        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> { while(openKey.consumeClick()) client.setScreen(new AudioPlayerScreen(client.screen)); while(pauseKey.consumeClick()) AUDIO.togglePause(); while(stopKey.consumeClick()) AUDIO.stop(); });
        ClientLifecycleEvents.CLIENT_STOPPING.register(client -> { CONFIG.config().volume=AUDIO.volume(); CONFIG.save(); AUDIO.close(); });
